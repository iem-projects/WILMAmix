...
class AudioMeter:
    def __init__(self):
        self.name = 'mymeter'
        #pipestring='jackaudiosrc client-name="WILMAmeter" ! audio/x-raw-float,channels=4 ! audioconvert ! level name='+self.name+' ! fakesink'
    ...
    def _handler(self, bus, message, data):
        if (gst.MESSAGE_ELEMENT is message.type) and (message.src.get_name() == self.name):
            self.levels = message.structure['peak']
        return True
