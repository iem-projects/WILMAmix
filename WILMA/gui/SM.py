...
import WILMA, WILMA.utils
import WILMA.net.osc as osc
...
class SM(QtGui.QGroupBox):
    ...
    def __init__(self, parent=None, name="SMi", constants={}, confs=None, maxwidth=None):
        ...
        self.connection = WILMA.net.client(config['address'], config['port'], oscprefix='/'+oscprefix, type=constants['/protocol'])
    ...
    def faderSet(self, value):
        gain=WILMA.utils.SCALE(value, self.fader.minimum(), self.fader.maximum(), 0., 1., True)
    ...
    def faderCb(self, msg, src):
        if len(msg)<3: ## that's only 'address' and 'typetags', no data
            return
        gainF=msg[2]
        gain=WILMA.utils.SCALE(gainF, 0., 1., self.fader.minimum(), self.fader.maximum(), True)
